...
wb.save('r1_mom_bollinger_follow_'+filename+'.xlsx')
...
for i in range(len(df_com)):
    ...
    if ((df_l3 - df_g3* 0.2) < a3 < (df_l3 + df_g3* 0.2)) or ((df_l2 - df_g2* 0.2) < a2 < (df_l2 + df_g2* 0.2)) :
        ...
        #현재가가 직전 2일간 최대값보다 크고 60이평선 1.1배 이하이며 60이평선이 상승중인 경우
        if df.iloc[-1]['Close'] > a1 and df.iloc[-1]['Close'] < df.iloc[-1]['MA60']*1.1 and df.iloc[-1]['MA60'] > df.iloc[-5]['MA60']: 
...
df_1 = pd.read_excel('r1_mom_bollinger_follow_'+filename+'.xlsx') 
df_b_f = df_1.sort_values(by = 'rise_margin(%)', ascending= False) # gap_close_ratio(%) 기준 올림차순으로 sorting
df_b_f.to_excel('r1_mom_bollinger_follow_'+filename+'.xlsx')
